...
rsq.frommean <- mean((atest$price-mean(atest$price))^2)
print(sprintf("We got rsq %.2f and the prediction from the mean is %.2f", rsq.rndfor, rsq.frommean))
```
...
yhat.btree <- predict(fit.btree, atest, n.trees = gbm.perf(fit.btree, plot.it = FALSE)) 
rsq.btree <- mean((yhat.btree - air.test_output) ^ 2)
rsq.btree
```
